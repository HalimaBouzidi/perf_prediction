...
if __name__ == "__main__" :
    ...
    with tf.Session(graph=tf.Graph()) as sess:
        ...
        if (model_name == 'Inception-v1') :
            ...
            net = EfficientNetB5(input_tensor = tf.ones(shape=input_shape, dtype='float32'), weights=None)

        elif (model_name == 'EfficientNet-B6') :
            from efficientnet.tfkeras import EfficientNetB6
            net = EfficientNetB6(input_tensor = tf.ones(shape=input_shape, dtype='float32'), weights=None)  

        elif (model_name == 'EfficientNet-B7') :
            from efficientnet.tfkeras import EfficientNetB7
            net = EfficientNetB7(input_tensor = tf.ones(shape=input_shape, dtype='float32'), weights=None) 
            ...
            ghost = GhostModel(1000,int(args.image_size),3)
            net = ghost.model
        ...
        elif (model_name == 'ResNet-1004') :
            from resnet_cifare_10 import resnet_v1
            net = resnet_v1(input_shape=(int(args.image_size), int(args.image_size), 3), full_shape=input_shape, depth=1004, num_classes=10)
            ...
            from resnet_cifare_10 import resnet_v2
            model = resnet_v2(input_shape=(int(args.image_size), int(args.image_size), 3), full_shape=input_shape, depth=1001, num_classes=10)

        elif (model_name == 'VGG-16') :
            from keras.applications.vgg16 import VGG16
            net = VGG16(input_tensor = tf.ones(shape=input_shape, dtype='float32')) 

        elif (model_name == 'VGG-19') :
            from keras.applications.vgg19 import VGG19
            net = VGG19(input_tensor = tf.ones(shape=input_shape, dtype='float32')) 

        elif (model_name == 'NASNet-Mobile') :
            from keras.applications.nasnet import NASNetMobile
            NASNetMobile = Classifiers.get('nasnetmobile')[0]
            net = NASNetMobile(input_tensor = tf.ones(shape=(1,240,240,3), dtype='float32')) 

        elif (model_name == 'NASNet-Large') :
            from keras.applications.nasnet import NASNetLarge
            NASNetLarge = Classifiers.get('nasnetlarge')[0]
            net = NASNetLarge(input_tensor = tf.ones(shape=input_shape, dtype='float32')) 
