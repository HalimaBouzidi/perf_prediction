...
if __name__ == "__main__" :
    ...
    models_infos = { 
        ...
        "GhostNet" : {"path" : "./Models/GhostNet", "tf_graph" : "GhostNet-tf-graph"},
    ...
    if(model_name == 'DenseNet-121' or model_name == 'DenseNet-161' or model_name == 'DenseNet-169' or model_name == 'DenseNet-201' or model_name == 'DenseNet-264') :
        ...
        if (model_name == 'ShuffleNet0.5-v2') :
            ...
            model = ShuffleNetV2(include_top=True, input_tensor=input_tensor, scale_factor=0.5, bottleneck_ratio=1)
            model_frozen_graph  = model_frozen_graph+'_'+args.input_size
            saved_model_json = model_info['path']+'/Saved-Model/'+args.model_name+'_'+args.input_size+'.json'
        elif (model_name == 'ShuffleNet1.0-v2') :
            input_tensor = Input(shape=(int(args.input_size), int(args.input_size), 3))
            model = ShuffleNetV2(include_top=True, input_tensor=input_tensor, scale_factor=1.0, bottleneck_ratio=1)
            model_frozen_graph  = model_frozen_graph+'_'+args.input_size
            saved_model_json = model_info['path']+'/Saved-Model/'+args.model_name+'_'+args.input_size+'.json'
        elif (model_name == 'ShuffleNet1.5-v2') :
            input_tensor = Input(shape=(int(args.input_size), int(args.input_size), 3))
            model = ShuffleNetV2(include_top=True, input_tensor=input_tensor, scale_factor=1.5, bottleneck_ratio=1)
            model_frozen_graph  = model_frozen_graph+'_'+args.input_size
            saved_model_json = model_info['path']+'/Saved-Model/'+args.model_name+'_'+args.input_size+'.json'
        elif (model_name == 'ShuffleNet2.0-v2') :
            input_tensor = Input(shape=(int(args.input_size), int(args.input_size), 3))
            model = ShuffleNetV2(include_top=True, input_tensor=input_tensor, scale_factor=2.0, bottleneck_ratio=1)
            model_frozen_graph  = model_frozen_graph+'_'+args.input_size
            saved_model_json = model_info['path']+'/Saved-Model/'+args.model_name+'_'+args.input_size+'.json'
    ...
    elif (model_name == 'ResNet-20' or model_name == 'ResNet-32' or model_name == 'ResNet-44' or model_name == 'ResNet-56' or model_name == 'ResNet-110' or model_name == 'ResNet-164' or model_name == 'ResNet-164' or model_name == 'ResNet-1001') :
        ...
        if (model_name == 'ResNet-20') :
            ...
            model_frozen_graph  = model_frozen_graph+'_'+args.input_size
        elif (model_name == 'ResNet-1001') :
            model = resnet_v1(input_shape=(int(args.input_size), int(args.input_size), 3), depth=1001, num_classes=10)
            model_frozen_graph  = model_frozen_graph+'_'+args.input_size
            saved_model_json = model_info['path']+'/Saved-Model/json_files/'+args.model_name+'_'+args.input_size+'.json'
        ...
        if (model_name == 'ResNet-20V2') :
            ...
            model = resnet_v2(input_shape=(int(args.input_size), int(args.input_size), 3), depth=1001, num_classes=10)
            model_frozen_graph  = model_frozen_graph+'_'+args.input_size
            saved_model_json = model_info['path']+'/Saved-Model/json_files/'+args.model_name+'_'+args.input_size+'.json'

    elif (model_name == 'GhostNet') :
        from ghostnet_ import GhostModel
        ghost = GhostModel(1000,None,3)
        model = ghost.model
