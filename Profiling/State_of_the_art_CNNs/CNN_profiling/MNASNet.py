...
def MnasNet(n_classes=1000, input_shape=(224, 224, 3), full_shape=(1, 224, 224, 3), alpha=1):
	inputs = layers.Input(shape=input_shape, batch_size=1, dtype='float32')
    
	#inputs = layers.Input(tensor=tf.ones(shape=full_shape, dtype='float32')) # change to it where FLOPS have to be calculated
