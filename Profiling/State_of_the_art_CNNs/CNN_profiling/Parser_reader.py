...
if __name__ == "__main__" :
    ...
    csv_file = "./Models/"+model_name+"/csv_files/"+model_name+"_"+args.image_size+".csv"
    ...
    with open(csv_file, newline='') as csvfile :
        ...
        for idx, row in enumerate(csv_reader, start=0) :
            if (idx !=0) :
                layers_count += 1
                if(row[1]=='Conv2D' or row[1]=='Conv2DTranspose' or row[1]=='DepthwiseConv2D') :
                    conv_count += 1
                    split_1 = row[2].split(" ")
                    ...
                    weighted_sum_neurones += ((width_1*width_2*kernel_size*depth_1)+(width_2*width_2*kernel_size*depth_1))*int(row[5])
                    sum_params_conv += int(row[11])
    ...
    with open('./Model_parser_info_new_10.csv', 'a', newline='') as file :
