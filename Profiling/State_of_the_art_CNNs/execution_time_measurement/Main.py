...
def image_to_tensor(image_path, image_size, model_name, not_batch) :
    ...
    if(model_name == 'DenseNet-121' or model_name == 'DenseNet-169' or model_name == 'DenseNet-201') :
    ...
    elif (model_name == 'Basic_3' or model_name == 'Basic_4') :
        from keras.applications.imagenet_utils import preprocess_input
        x = preprocess_input(x)
...
if __name__ == "__main__" :
    ...
    with tf_sess :
        ...
        "AlexNet" : {"path" : "./Models/AlexNet", "tf_graph" : "AlexNet-tf-graph",
            "output_names" : "dense_4/Softmax", "input_names" : "input_1"},
        ...
        with TF_graph.as_default():
            ...
            #with open('./execution_time_new.csv', 'a', newline='') as file :
            #    writer = csv.writer(file)
            #    writer.writerow([args.model_name, (int(args.image_size), int(args.image_size), 3), min(times_[10:]), max(times_[10:]), mean(times_[10:])])

            mean_delta = np.array(times_[10:]).mean()
            fps = 1 / mean_delta
            print('inference time for the batch (ms): ', int(mean_delta*1000))
            print('FPs = ', fps)
